    ...
    return (
        <ul className="todo__list" onClick={onClick}>
            {items.map((item) => (
                <li
                    key={`${item.id}`}
                    className={classNames('todo__list-item', item.className, { 'is-active' : item.active })}
                >
                    <button className="todo__list-btn">
                        { item.icon ? (
                            ...
                            <Badge color={DB.colors.filter(color => color.id === item.colorId)[0].hex} />
                        )}
                        <span className="todo__list-btn-text">{item.name}</span>
                        { isRemovable && (
